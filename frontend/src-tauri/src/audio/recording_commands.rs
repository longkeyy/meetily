...
fn forward_source_activity_events<R: Runtime>(
    app: AppHandle<R>,
    mut receiver: tokio::sync::mpsc::UnboundedReceiver<super::recording_state::SourceActivityEvent>,
) {
    tauri::async_runtime::spawn(async move {
        while let Some(event) = receiver.recv().await {
            if let Err(error) = app.emit("audio-source-activity", event) {
                warn!("Failed to emit audio source activity: {}", error);
            }
        }
    });
}
...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let (transcription_receiver, source_activity_receiver) = manager
        .start_recording(
            ...
            system_device,
            auto_save,
            assistant_settings.live_checkpoint_ms(),
        )
        .await
        .map_err(|e| format!("Failed to start recording: {}", e))?;
    forward_source_activity_events(app.clone(), source_activity_receiver);
